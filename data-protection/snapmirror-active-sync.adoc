...
Aunque los procedimientos de System Manager para configurar SnapMirror active sync son diferentes en los sistemas ASA r2 que en los sistemas NetApp FAS, AFF y ASA que ejecutan la personalidad unificada de ONTAP, los requisitos, la arquitectura y el funcionamiento de SnapMirror active sync son los mismos. link:https://www.netapp.com/pdf.html?item=/media/138366-sb-3457-san-disaster-recovery-netapp-asa.pdf["Obtenga más información sobre la recuperación ante desastres con SnapMirror Active Sync en su sistema ASA r2"^]

En los sistemas ASA r2, SnapMirror active sync admite configuraciones activas/activas simétricas. En una configuración activa/activa simétrica, ambos sitios pueden acceder al almacenamiento local para E/S activas. Más información sobre link:https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/architecture-concept.html#symmetric-activeactive["configuraciones simétricas activas/activas"^].
...
* A partir de ONTAP 9.19.1, la Protección Autónoma contra Ransomware con Inteligencia Artificial (ARP/AI) es compatible con las unidades de almacenamiento primarias de lectura y escritura en relaciones SAN de sincronización activa de SnapMirror en sistemas ASA r2. ARP/AI se ejecuta en las unidades de almacenamiento primario activas, crea instantáneas ARP solo para el almacenamiento primario y ofrece detección de ransomware basada en aprendizaje automático para esas unidades de almacenamiento. Para ver los pasos para habilitar ARP/AI, consulta link:../secure-data/enable-anti-ransomware-protection.html["Activa la protección autónoma contra el ransomware con IA"].


.Antes de empezar
Si estás ejecutando ONTAP 9.19.1 o una versión posterior y habilitando SnapMirror active sync para un host AIX, deberías link:prepare-aix-snapmirror-active-sync.html["preparar el host AIX"].

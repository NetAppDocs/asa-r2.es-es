...
[role="lead"]
Para optimizar el rendimiento en tu sistema ASA r2, puede que necesites modificar tus unidades de almacenamiento para aumentar su capacidad, actualizar las políticas de QoS o cambiar los hosts que están asignados a las unidades. Por ejemplo, si se añade una nueva carga de trabajo de aplicación crítica a una unidad de almacenamiento existente, puede que necesites cambiar la política de Quality of Service (QoS) aplicada a la unidad de almacenamiento para admitir el nivel de rendimiento necesario para la nueva aplicación.

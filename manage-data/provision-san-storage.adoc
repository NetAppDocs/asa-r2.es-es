...
Una unidad de almacenamiento ASA r2 tiene una capacidad máxima de 128 TB. Ver ellink:https://hwu.netapp.com/["NetApp Hardware Universe"^] para conocer los límites de almacenamiento más actuales para los sistemas ASA r2.

Usted agrega y asigna iniciadores de host a la unidad de almacenamiento como parte del proceso de creación de la unidad de almacenamiento.  También puedeslink:provision-san-storage.html#add-host-initiators["agregar"] ylink:provision-san-storage.html#map-the-storage-unit-to-a-host["asignar"] iniciadores de host después de crear las unidades de almacenamiento.
...
Las unidades de almacenamiento se aprovisionan de forma ligera por defecto. El aprovisionamiento ligero permite que la unidad de almacenamiento crezca hasta el tamaño asignado pero no reserva el espacio por adelantado. El espacio se asigna dinámicamente a partir del espacio libre disponible según sea necesario. Esto te permite lograr una mayor eficiencia de almacenamiento _sobreaprovisionando_ tu espacio disponible. Por ejemplo, supón que tienes 1 TB de espacio libre y necesitas crear cuatro unidades de almacenamiento de 1 TB. En vez de añadir inmediatamente 3 TB de capacidad de almacenamiento adicional a tu sistema, puedes crear las unidades de almacenamiento, monitorear la utilización del espacio y aumentar tu capacidad de almacenamiento a medida que las unidades de almacenamiento consumen espacio real. Más información sobre link:https://docs.netapp.com/us-en/ontap/concepts/thin-provisioning-concept.html["aprovisionamiento fino"^].
...
CAUTION: Si estás creando más de una unidad de almacenamiento, cada unidad se crea con la misma capacidad. Multiplica el número de unidades de almacenamiento que estás creando por la capacidad deseada para asegurarte de que tienes suficiente espacio utilizable. Si no tienes suficiente espacio libre y decides sobreaprovisionar, monitorea de cerca la utilización para evitar quedarte sin espacio y perder datos.

+
A partir de ONTAP 9.19.1, si estás creando un LUN AIX que planeas habilitar para la protección de sincronización activa de SnapMirror, en *Información del host*, selecciona *local* para el *Modo de acceso*. Esto es necesario para habilitar la conmutación por error transparente de aplicaciones.
...
+
Si no hay un grupo de hosts preexistente para los hosts que se ejecutan en el sistema operativo que especificaste, ONTAP crea un nuevo grupo de hosts automáticamente.
...
+
Formatee el NQN del host como <nqn.yyyy-mm> seguido de un nombre de dominio completo.  El año debe ser igual o posterior a 1970.  La longitud máxima total debe ser 223.  Un ejemplo de un iniciador NVMe válido es nqn.2014-08.com.example:string

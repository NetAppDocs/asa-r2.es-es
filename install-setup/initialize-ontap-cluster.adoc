...
En ciertos casos, es posible que necesites link:../learn-more/cli-support.html#set-up-an-ontap-asa-r2-cluster-using-the-cli["Utilice la interfaz de línea de comandos (CLI) de ONTAP para configurar su clúster"]. Debe utilizar la CLI, por ejemplo, si sus protocolos de seguridad no le permiten conectar una computadora portátil a sus conmutadores de administración o si está utilizando un sistema operativo que no sea Windows.

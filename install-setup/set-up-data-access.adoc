...
Si necesita migrar la carga de trabajo de su máquina virtual desde un sistema de almacenamiento ASA a un sistema de almacenamiento ASA r2, NetApp recomienda que utilicelink:https://www.vmware.com/products/cloud-infrastructure/vsphere-foundation["VSphere vMotion de VMware"^] para realizar una migración en vivo y sin interrupciones de sus datos.

Las unidades de almacenamiento ASA r2 tienen aprovisionamiento fino de manera predeterminada.  Al migrar su carga de trabajo de VM, los discos virtuales (VMDK) también deben tener aprovisionamiento fino.
...
* Obtenga más información sobrelink:https://kb.netapp.com/on-prem/ontap/da/SAN/SAN-KBs/How_to_setup_Broadcom_VMware_ESXi_iSCSI_MPIO_with_ONTAP_SAN_ASA["Cómo configurar Broadcom VMware ESXi iSCSI MPIO con sistemas de almacenamiento ONTAP SAN ASA"^] .
...
A partir de ONTAP 9.19.1, FLI es compatible con el protocolo iSCSI para la conectividad backend.

* Más información sobre el enlace: https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html[Migraciones ONTAP FLI^].
